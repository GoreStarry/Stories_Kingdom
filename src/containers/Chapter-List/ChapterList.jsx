          ...
          <div className={styles.box__special_link}>
            {last_edited_article && (
              <Link to={`/editor/${story_id}/${last_edited_article}`}>
                <Button secondary>進入上一次編輯</Button>
              </Link>
            )}
            <Link to={`/editor/${story_id}/${_last(articleOrder).id}/`}>
              <Button secondary>進入最尾頁</Button>
            </Link>
          </div>
          <List divided verticalAlign="middle">
            {articles[story_id] &&
              stories &&
              articleOrder.map(article_detail => {
                const article_id = article_detail.id;
                const article = articles[story_id][article_id];

                return article.chapterName || article.outline ? (
                  <ChapterCard
                    key={article_id}
                    article={article}
...
function mapDispatchToProps(dispatch) {
  return {
    actions: {
      ...
      editArticle: (article_id, editedState) =>
        dispatch(actionEditArticle(article_id, editedState))
    }
  };
}
