      ...
      .map((article, index) => {
        if (article == "now_article") {
          ...
          return {
            ...
            editorState: articles[id].draftContent
              ? EditorState.createWithContent(
                  convertFromRaw(JSON.parse(articles[id].draftContent))
                )
              : emptyEditorStateForDisplay
          };
        } else {
          // when now_article at first page or last page
          return article;
        }
      })
      .filter(article => {
