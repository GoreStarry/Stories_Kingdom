...
import {
  blockRenderMap,
  blockStyleFn
} from "./draft-settings-map/blockRenderMap.js";
...
class MotionEditor extends PureComponent {
  ...
  render() {
    ...
    return (
      <Motion
        key={article_id}
        style={{ x: spring(position == "next" && -innerWidth) }}
      >
        {style => {
          if (position) {
            // next & prev Editor (just for display)
            return (
              <div
                className={styles.box__editor}
                style={{
                  transform: `translate3d(${style.x}px, 0, 0)`,
                  textAlign: articleAlign
                }}
              >
                <Editor
            ...
            // main Editor can be realy edited
            return (
              <div
